...
use godot::classes::PhysicsShapeQueryParameters3D;
...
impl PollingResult {
    ...
    pub fn from_godot_thinker(thinker: &Gd<GodotThinker>) -> Self {
        Self {
            space_rid: PhysicsServer3D::singleton().body_get_space(thinker.bind().character_body.as_ref().unwrap().get_rid()),
            shape_ainodes_rid: thinker.bind().ainodes_detection_shape.as_ref().unwrap().get_rid(),
            shape_vision_rid: thinker.bind().vision_detection_shape.as_ref().unwrap().get_rid(),
            transform: thinker.bind().character_body.as_ref().unwrap().get_global_transform(),
            ainodes: None,
            visible_targets: None
        }
    }

    fn poll<T>(&mut self, collision_mask: u32, shape_res_rid: Rid, collide_with_bodies: bool, collide_with_arenas: bool, mapping: fn(Dictionary) -> Option<T>) -> Vec<T> {
        let mut space = PhysicsServer3D::singleton().space_get_direct_state(self.space_rid).expect("no space?!");
        ...
        space.intersect_shape(query_parameters)
            .iter_shared()
            .filter_map(mapping)
            .collect()
    }
    ...
    pub fn get_visible(&mut self) -> Option<&Vec<VisibleTarget>> {
                    ...
                    |d|
                        {
                            ...
                            let shape_rid = PhysicsServer3D::singleton().area_get_shape(collider_rid, shape_index);
                            let shape_data = PhysicsServer3D::singleton().shape_get_data(shape_rid).to::<Dictionary>();
                            let radius = shape_data.get("radius").map(|v| v.to::<f32>())?;
                            let height = shape_data.get("height").map(|v| v.to::<f32>())?;
                            let area_global_transform = PhysicsServer3D::singleton().area_get_transform(collider_rid);
        ...
        // return None if cached result is empty
        let is_empty = self.visible_targets.as_ref().map(|v| v.is_empty()).unwrap_or(true);
        if is_empty {
            return None
        }
...
pub fn process_thinker(thinker: &mut Thinker, delta: f64, ainodes: &Arc<RwLock<HashMap<u32, Arc<Mutex<AINode>>>>>) {
    let mut polls = PollingResult::from_godot_thinker(thinker.base.as_ref().unwrap());
    let base = thinker.base.as_mut().unwrap();
    let Ok(mut shared_guard) = thinker.shared.lock() else {panic!("mutex failed!")};
    ...
    let mut sensor_args = SensorArguments {
        ...
        head_position: base.bind().head_position.as_ref().map(|hp| hp.get_global_position()).unwrap_or(Vector3::ZERO),
        thinker_forward_axis: base.bind().character_body.as_ref().map(|ch| ch.get_global_basis().col_c()).unwrap_or(Vector3::ZERO),
