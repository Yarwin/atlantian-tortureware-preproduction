...
use godot::engine::{CharacterBody3D, MeshInstance3D};
use godot::prelude::*;
...
fn combine_tables_and_get_velocity(interest_table: &SteeringTable, danger_table: &SteeringTable, base_vec: Vector3) -> Vector3 {
...
fn seek(target: Vector3, base_vec: Vector3) -> SteeringTable {
    unsafe {
        ...
        for (i, item) in &mut arr[..].iter_mut().enumerate() {
            let weight = base_vec.rotated(Vector3::UP, ANGLES[i])
                .normalized()
                .dot(target).max(0.0);
            item.write(weight);
        }
...
fn avoid(interest_table: &SteeringTable, old_danger_table: &SteeringTable, avoidance_radius: f32, caster: &mut Gd<CharacterBody3D>, base_vec: Vector3, agent_radius: f32) -> SteeringTable {
    ...
    if matches!(caster_rid, Rid::Invalid) { return *old_danger_table };
    let Some(mut direct_space) = PhysicsServer3D::singleton().space_get_direct_state(space_rid) else {return  *old_danger_table };
    ...
    for (i, angle) in ANGLES.iter().enumerate() {
        ...
        debug_node.set_global_position(
            caster_pos +
                (base_vec * avoidance_radius).rotated(Vector3::UP, *angle)
                + UP_OFFSET
            );
        ...
        ray_params.set_to(
            caster_pos +
            (base_vec * avoidance_radius).rotated(Vector3::UP, *angle).normalized()
            + UP_OFFSET
        );
        let intersection_result = direct_space.intersect_ray(ray_params.clone());
        if let Some(colpos) = intersection_result.get("position").map(|v| v.to::<Vector3>()) {
            let distance = caster_pos.distance_to(colpos - UP_OFFSET);
            if distance >= agent_radius {
                let current_danger = (1.0 - (distance / (avoidance_radius - agent_radius))).max(0.0);
                // interpolate the result over few frames to prevent "cycling"
                danger_table[i] = old_danger_table[i].lerp(current_danger, 0.33);
                continue
            }

            let is_colliding_with_other_character = intersection_result.get("collider").map(|v| v.try_to::<Gd<CharacterBody3D>>().is_ok()).unwrap_or(false);
...
pub fn rotate(character: &mut Gd<CharacterBody3D>, target: Vector3, rotation_speed: f32, delta: f64) {
    let ground_offset = (character.get_global_transform().origin * Vector3::UP).y;
    let lateral_plane = Plane::new(Vector3::UP, ground_offset);
    let look_at_transform = character.get_transform().looking_at(lateral_plane.project(target), Vector3::UP, true);
    let new_transform = character.get_transform().interpolate_with(look_at_transform, rotation_speed * delta as f32);
...
pub fn navigate(mut navigation_arguments: NavigationArguments, delta: f64) {
    ...
    let Some(mut character) = bind.character_body.clone() else { return; };
    ...
    let mut desired_velocity = navigation_arguments.blackboard.desired_velocity.take().unwrap_or(Vector3::ZERO);
    ...
    if !desired_velocity.is_zero_approx() {
        ...
        navigation_arguments.navigation_data.danger_table = avoid(&interest_table, old_danger_table, bind.avoidance_detection_radius, &mut character, forward_vec, bind.agent_radius);
        let avoidance = combine_tables_and_get_velocity(&interest_table, &navigation_arguments.navigation_data.danger_table, forward_vec);
        let mut debug_node = character.get_node_as::<MeshInstance3D>("Debug/DebugDir");
        debug_node.set_global_position(character.get_global_position() + desired_velocity.length() * avoidance.normalized());
