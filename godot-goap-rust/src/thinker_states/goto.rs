...
impl GotoState {
    fn get_target_pos(&self) -> Vector3 {
        match self.destination {
            ...
            Destination::Node(_id) => {todo!()}
            Destination::Character(_id) => {todo!()}
        }
    }
}
...
impl ThinkerState for GotoState {
    ...
    fn enter(&mut self, mut args: StateArguments) {
        let mut bind = args.base.bind_mut();
        let Some(nav_agent) = bind.navigation_agent.as_mut() else { return; };
    ...
    fn physics_process(&mut self, delta: f64, mut args: StateArguments) {
        let mut bind = args.base.bind_mut();
        let Some(character) = bind.character_body.clone() else { return; };
        let Some(mut anim_tree) = bind.animation_tree.clone() else {return; };
        ...
        let (speed, acceleration) = match args.blackboard.walk_speed {
            ...
            MovementSpeed::Walk => {(bind.movement_speed * bind.walk_speed_mod, bind.acceleration * bind.walk_speed_mod)}
        ...
        if nav_agent.is_navigation_finished() {
            ...
            self.finished = true;
            args.world_state[WorldStateProperty::IsNavigationFinished] = Some(WSProperty::Truth(true));
            return;
        }
        ...
        let direction: Vector3 = character.get_global_position().direction_to(lateral_plane.project(next_path_position));
        if direction.length_squared().is_zero_approx() {
            return;
        }
        let look_target = lateral_plane.project(next_path_position);
        let update_look_target = args.blackboard.rotation_target.as_ref().map(|rt| !(*rt - look_target).is_zero_approx()).unwrap_or(true);
        ...
        //     dot_product = 1.0;
        // }
        let movspeed = velocity.move_toward(direction * speed * dot_product, acceleration * delta as f32);
