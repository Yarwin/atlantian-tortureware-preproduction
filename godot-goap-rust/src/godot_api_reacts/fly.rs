...
impl GameEffectInitializer for FlyGameEffect {
    fn build(&self, act_context: &Dictionary, context: &Dictionary) -> Option<GameEffectProcessor> {
        let Some(flier) = context.get("reactor").map(|v| v.to::<Gd<RigidBody3D>>()) else {panic!("no reactor!")};
        let force = self.force;
        let force_multiplier = act_context.get("force").map(|v| v.try_to::<f32>().unwrap_or(1.0)).unwrap_or(1.0);
        let Some(direction) = context.get("direction").map(|v| -(v.to::<Vector3>())) else {panic!("no direction")};
...
impl GameEffect for Fly {
    fn execute(&mut self) -> EffectResult {
        self.target.take().unwrap().apply_impulse_ex(self.direction * self.force).position(self.at).done();
        EffectResult::Free
    }
}