...
impl GrabGameEffect {
    fn get_grabber(from: &Variant) -> Option<Gd<GrabNode>> {
        match from.try_to::<Gd<GrabNode>>() {
            Ok(g) => { Some(g) }
            Err(e) => {
                match e.value()?.try_to::<Gd<PlayerController>>() {
                    Ok(pc) => {
                        Some(pc.bind().grab_node.clone())
                    }
                    Err(_) => { None }
                }
            }
...
impl GameEffect for Grab {
    fn execute(&mut self) -> EffectResult {
        let Some(mut grabber) = GrabGameEffect::get_grabber(&self.grabber) else {return EffectResult::Failed};
        grabber.bind_mut().attach_rigid(self.grabbed.take().unwrap().cast::<RigidBody3D>());
        EffectResult::Free
    }
