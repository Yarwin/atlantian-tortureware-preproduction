use godot::prelude::*;
use godot::classes::{AnimationPlayer, Control, GpuParticles3D, Marker3D, PackedScene, ParticleProcessMaterial, PhysicsRayQueryParameters3D, Texture2D};
...
impl SpreadGun {
    ...
    fn reload_with_concrete_ammo(&mut self, eq_component: &mut SpreadGunItemComponent, reloaded: &mut u32, to_reload: &mut u32, player_inventories_ids: Vec<u32>) {
        let accepted_ammo_item_type = {
            let Some(ammo_type) = eq_component.current_ammo.as_ref().map(|a| a.bind()) else {return;};
        ...
        for inventory in player_inventories_ids {
            let items = InventoryManager::singleton().bind().get_items_of_the_same_type(inventory, accepted_ammo_item_type.clone());
            for item in items.iter_shared() {
                ...
                InventoryManager::singleton().bind_mut().reduce_stack(item, to_remove);
                *reloaded += to_remove;
                *to_reload -= to_remove;
            }
        }
    ...
    fn reload(&mut self) {
        let eq_comp = self.eq_component.as_mut().map(|c| unsafe { &mut **c }).unwrap();
        let eq_data = eq_comp.data.bind();
        let mut to_reload = eq_data.mag_size - eq_comp.ammo_count;
        drop(eq_data);

        if to_reload == 0 {return};
        ...
        if eq_comp.current_ammo.is_some() {
            self.reload_with_concrete_ammo(eq_comp, &mut reloaded, &mut to_reload, player_inventories);
        }

        if reloaded > 0 {
            let new_ammo_count = eq_comp.ammo_count + reloaded;
            let on_animation_finished = self.base().callable("on_reload_animation_finished").bindv(varray![new_ammo_count.to_variant()]);
            self.animation_player.play_ex().name("reload".into()).done();
            ...
            self.base_mut().emit_signal("gun_status_changed".into(), &["reloading".to_variant()]);
        }
    }

    fn get_act_react_context(&mut self, collision: Dictionary, ammo: &Gd<SpreadGunAmmo>) -> Option<(Gd<ActReactResource>, Gd<ActReactResource>, Dictionary)> {
    ...
    fn apply_hitscan_collision(&mut self, collision: Dictionary, ammo: &Gd<SpreadGunAmmo>) {
        if let Some((actor, reactor, context)) = self.get_act_react_context(collision.clone(), ammo) {
            ActReactExecutor::singleton().bind_mut().react(actor, reactor, context);
        }
        let Some(col) = collision.get("position") else {return;};
        let Some(normal) = collision.get("normal") else {return;};
        // draw some trails
        if let Some(particle) = ammo.bind().trail_particle.as_ref() {
            let mut particle = particle.instantiate().unwrap().cast::<GpuParticles3D>();
            let mat = particle.get_process_material().unwrap().cast::<ParticleProcessMaterial>();
            let v = mat.get("initial_velocity_min".into()).to::<f64>();
            let lifetime = col.to::<Vector3>().distance_to(self.muzzle.get_global_position()) as f64 / v;
            particle.set_lifetime(lifetime);
            self.muzzle.add_child(&particle);
        }
        GameSys::singleton().emit_signal("new_hitscan_collision_registered".into(), &[col, normal]);
    }

    fn shoot(&mut self) {
        let ammo = unsafe { (**self.eq_component.as_mut().unwrap()).current_ammo.as_ref().unwrap()};
        let mut space_state = self.base().get_world_3d().unwrap().get_direct_space_state().unwrap();
        ...
        for spread in spreads.iter_shared() {
            ...
            query.set_exclude(self.excluded.clone());
...
impl INode3D for SpreadGun {
    fn ready(&mut self) {
        let player = self.base().get_tree().unwrap().get_first_node_in_group("player".into()).unwrap().cast::<CharacterController3D>();
        self.excluded.push(player.get_rid());
        self.animation_player.play_ex().name("prepare".into()).done();
        self.player.init(
            self.base().get_tree().unwrap().get_first_node_in_group("player_controller".into()).unwrap().cast::<PlayerController>()
        );
    }
}
...
impl SpreadGun {
    ...
    fn on_reload_animation_finished(&mut self, _a_name: StringName, new_ammo_count: u32) {
        let Some(eq_component) = self.eq_component.as_mut().map(|c| unsafe { &mut **c }) else { return; };
        eq_component.ammo_count = new_ammo_count;
        let Some(item) = self.item.as_mut() else {return;};
        item.emit_signal("updated".into(), &[]);
        self.base_mut().emit_signal("gun_status_changed".into(), &[GString::default().to_variant()]);
        self.base_mut().emit_signal("reloaded".into(), &[]);
    }
