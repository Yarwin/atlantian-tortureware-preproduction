...
impl<'a, T: PlanAction<U>, U> PlanNode<'a, T, U> {
    ...
    pub fn child(parent_state: WorldState, action_type: &'a T, action_arguments: &'a U) -> PlanNode<'a, T, U> {
        let mut child = PlanNode {
            current_state: parent_state.clone(),
            action: Some(action_type),
            extra_action_arguments: PhantomData
        };
        child.current_state.apply_world_state(action_type.get_action_effects(action_arguments));
        child
    }

    pub fn neighbours(&self, actions: &'a [T], action_arguments: &'a U) -> Vec<(PlanNode<'a, T, U>, u32)> {
        let actions = actions
            .iter()
            .filter_map(|action| {
                // effects are already satisfied in current state
                if action.get_action_effects(action_arguments).count_unsatisfied_world_state_props(&self.current_state) == 0 {
                    return None;
                }

                // preconditions are not met
                if action.get_action_preconditions().count_unsatisfied_world_state_props(&self.current_state) != 0 {
                    return None;
                }
                ...
                Some((PlanNode::child(self.current_state.clone(), action, action_arguments), action.get_action_cost(action_arguments)))
            }).collect();
        actions
    }
}

pub fn plan<'a, T: PlanAction<U>, U>(initial_state: &'a WorldState, goal_state: &'a WorldState, actions: &'a [T], action_arguments: &'a U) -> Option<Vec<&'a T>> {
    let start = PlanNode::initial(initial_state);
    let plan = astar(&start,
    ...
    if let Some((plan, _cost)) = plan {
        ...
        return Some(plan.into_iter().skip(1).map(|node| node.action.unwrap()).collect())
    }
    None
}
    ...
    impl TestCase {
        ...
        fn assert_plan(&self) {
            ...
            if let Some(action_list) = plan {
                let action_names: Vec<String> = action_list.iter().map(|&action| action.name.clone()).collect();
