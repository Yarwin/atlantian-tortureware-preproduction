...
impl InventoryUIGrid {
    ...
    pub fn global_coords_to_index(&self, coords: Vector2) -> Option<usize> {
        ...
        loop {
            ...
            let child = grid.get_child(midpoint).expect("logic error, no child with index {midpoint}").cast::<Control>();
            let child_rect = child.get_global_rect();
            if child_rect.has_point(coords) {
                return Some(midpoint as usize);
            }
            let row_equality = coords.y >= child_rect.position.y && coords.y <= child_rect.position.y + child_rect.size.y;
            if row_equality {
    ...
    pub fn initialize_grid(&mut self) {
        let Some(grid) = self.grid.as_mut() else {return;};
        let Some(inventory_agent) = self.inventory_agent.as_ref() else {return;};
        let Some(grid_cell_scene) = self.grid_cell_scene.as_ref() else {return;};
    ...
    pub fn init_inventory_agent(&mut self) -> bool {
        ...
        let Some(potential_agent) = self.base().get_tree().unwrap().get_first_node_in_group(self.inventory_group.clone()).map(|n| n.cast::<InventoryAgent>()) else {return false};
...
impl InventoryUIGrid {
    ...
    fn initialize_items(this: Gd<Self>) {
        let items = this.bind().inventory_agent.as_ref().expect("no items").bind().get_items();
    ...
    fn on_cell_size_calculated(&mut self, new_offset: f32) {
        ...
        let Some(margin_container) = self.margin_container.as_mut() else {return;};
        ...
        margin_container.set_offset(Side::BOTTOM, (vertical_offset + offset_top_diff).clamp(0.0, size.y / 2.0));
        ...
        self.base_mut().emit_signal("offset_set".into(), &[]);
    }

    #[func(gd_self)]
    pub fn append_item(mut this: Gd<Self>, mut item: Gd<InventoryUIItem>) {
        let on_resized = item.callable(StringName::from("resize_and_put"));
        this.connect_ex("offset_set".into(), on_resized).flags(CONNECT_DEFERRED).done();
        let on_inventory_switched = this.callable(StringName::from("on_item_removed_from_inventory")).bindv(array![item.to_variant()]);
        item.bind_mut().item.as_mut().unwrap().connect_ex("inventory_switched".into(), on_inventory_switched).flags(CONNECT_ONE_SHOT).done();
        item.bind_mut().current_inventory_ui = Some(this);
    }

    #[func(gd_self)]
    fn on_item_removed_from_inventory(mut this: Gd<Self>, _new_inventory_id: Variant, item: Gd<InventoryUIItem>) {
        let on_resized = item.callable(StringName::from("resize_and_put"));
        this.disconnect("offset_set".into(), on_resized);
        // let on_inventory_switched = this.callable(StringName::from("on_item_removed_from_inventory")).bindv(array![item.to_variant()]);
        // item.bind_mut().item.as_mut().unwrap().disconnect("inventory_switched".into(), on_inventory_switched);
    }
}
