...
impl FromGodot for ReceivedDamage {
    fn try_from_godot(via: Self::Via) -> Result<Self, ConvertError> {
        let Some(Ok(strength)) = via.get("strength").map(|v| v.try_to::<f64>()) else {
            return Err(ConvertError::default());
        };
        let Some(Ok(pain)) = via.get("pain").map(|v| v.try_to::<f64>()) else {
            return Err(ConvertError::default());
        };
        let Some(Ok(pos)) = via.get("position").map(|v| v.try_to::<Vector3>()) else {
            return Err(ConvertError::default());
        };
        let Some(Ok(normal)) = via.get("normal").map(|v| v.try_to::<Vector3>()) else {
            return Err(ConvertError::default());
        };
        let direction = via
            .get("direction")
    ...
    pub fn resolve_damage(&mut self, damage: ReceivedDamage) {
        if damage.strength > 0. {
            ...
            self.base_mut().emit_signal(
                "damage_taken",
                &[
                    damage.strength.to_variant(),
                    damage.pos.to_variant(),
                    damage.normal.to_variant(),
                ],
            );
        }
        if damage.pain > 0. {
...
impl INode for DamageReceptorComponent {
    fn physics_process(&mut self, delta: f64) {
        ...
        if self.hp < 0. {
            ...
            self.base_mut()
                .emit_signal("health_depleted", &[is_gib.to_variant()]);
        }

        if self.pain_taken > 0. {
            let is_immune_to_pain = self
                .time_since_last_pain_state
                .map(|lp| lp.elapsed().unwrap().as_secs_f64() < self.pain_resistance_time)
                .unwrap_or(false);
            if !is_immune_to_pain {
                ...
                if self.pain > self.pain_threshold {
                    ...
                    self.pain = 0.;
                    self.base_mut().emit_signal("pain_threshold_achieved", &[]);
                }
                self.time_since_last_pain = SystemTime::now();
            }
            self.pain_taken = 0.;
        } else if self.time_since_last_pain.elapsed().unwrap().as_secs_f64()
            > self.pain_recovery_cooldown
        {
