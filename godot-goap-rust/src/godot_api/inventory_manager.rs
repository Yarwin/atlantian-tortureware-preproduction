...
impl InventoryManager {
    ...
    pub fn get_items(&self, inventory_id: u32) -> Array<Gd<Item>> {
        ...
        for id in inventory.get_items() {
            items.push(self.items.get(&id).unwrap());
        }
        items
    }

    #[func]
    pub fn get_items_of_the_same_type(
        &self,
        inventory_id: u32,
        inventory_data: Gd<InventoryItemData>,
    ) -> Array<Gd<Item>> {
        let inventory = self.inventories.get(&inventory_id).unwrap();
        let mut items: Array<Gd<Item>> = Array::new();
        for id in inventory.get_items() {
            ...
            let Some(other_inventory_data) =
                item_bind.inventory.as_ref().map(|i| &i.inventory_data)
            else {
                panic!("no inventory data")
            };
...
impl InventoryManager {
    ...
    fn remove_item(&mut self, mut item: Gd<Item>) {
        ...
        if let Some(Some(inventory_id)) = item
            .bind()
            .inventory
            .as_ref()
            .map(|i| i.current_inventory_id)
        {
            self.inventories
    ...
    fn update_item_and_return_result(
        &mut self,
        result: Result<Gd<Item>, InventoryEntityResult>,
        inventory_id: u32,
    ) -> Result<Gd<Item>, InventoryEntityResult> {
        match result {
            Ok(mut item) => {
                let previous_inventory =
                    item.bind().inventory.as_ref().unwrap().current_inventory_id;
                let inventory_changed = previous_inventory
                    .map(|p_id| p_id != inventory_id)
                    .unwrap_or(false);
                if inventory_changed {
                    if let Some(inv) = self
                        .inventories
                        .get_mut(previous_inventory.as_ref().unwrap())
                    {
                        inv.remove_item(item.bind().id);
                    }
                    item.emit_signal("inventory_switched", &[inventory_id.to_variant()]);
                }
                item.bind_mut()
                    .inventory
                    .as_mut()
                    .unwrap()
                    .current_inventory_id = Some(inventory_id);
                if previous_inventory.is_none() {
                    item.emit_signal("inventory_switched", &[inventory_id.to_variant()]);
                    if let Some(inventory_agent) = self.inventory_agents.get_mut(&inventory_id) {
                        inventory_agent.emit_signal("new_item_created", &[item.to_variant()]);
                    }
                }
                Ok(item)
            }
    ...
    fn create_items_in_inventory(
        &mut self,
        item_to_spawn: Gd<ItemToSpawn>,
        inventory: Option<&mut InventoryEntity>,
        inventory_id: u32,
    ) -> bool {
        ...
        for mut item in builder {
            ...
            let item_to_insert = if item_to_spawn.bind().assign_position {
                ...
                let Ok(item) = inventory.try_insert_item_at(item, at_index) else {
                    return false;
                };
                item
            } else {
                // panics if there isn't space for given item in the inventory
                let Ok(item) = inventory.insert_at_first_free_space(item) else {
                    return false;
                };
                item
            };
            if let Some(ref mut agent) = inventory_agent {
                agent.emit_signal("new_item_created", &[item_to_insert.to_variant()]);
            }
    ...
    pub fn put_item_at_first_free_space(
        &mut self,
        item: Gd<Item>,
        inventory: Option<&mut InventoryEntity>,
        inventory_id: u32,
    ...
    pub fn create_item_in_inventory(
        &mut self,
        item_to_spawn: Gd<ItemToSpawn>,
        inventory_id: u32,
    ) -> bool {
    ...
    pub fn try_stack_item(&mut self, mut item: Gd<Item>, mut other: Gd<Item>) -> StackResult {
        ...
        let result = {
            ...
            let Some(other_inventory_component) = other_item_bind.inventory.as_mut() else {
                return StackResult::WrongType;
            };
    ...
    pub fn check_grid_cells(
        &self,
        item: Gd<Item>,
        inventory_id: u32,
        position_idx: usize,
    ) -> InventoryEntityResult {
        let Some(inventory) = self.inventories.get(&inventory_id) else {
            return InventoryEntityResult::WrongItemType(item);
        };
    ...
    pub fn move_item(
        ...
        let Some(inventory) = self.inventories.get_mut(&inventory_id) else {
        ...
        if let Some(other_item_id) = inventory.get_item_id_at(position_idx) {
            if other_item_id != item.bind().id {
                ...
                return match result {
                    StackResult::WrongType => Err(InventoryEntityResult::SpaceTaken(
                        vec![(position_idx, other_item_id)],
                        item,
                    )),
                    StackResult::Updated => {
                        other_item.base_mut().emit_signal("updated", &[]);
                        Ok(item)
                    }
    ...
    fn initialize_inventories(&mut self) {
        ...
        for mut to_create in to_drain.drain(..) {
            ...
            for item_to_spawn in to_create.agent.bind().items_to_spawn.iter_shared() {
                if !self.create_items_in_inventory(
                    item_to_spawn,
                    Some(&mut inventory),
                    inventory_id,
                ) {
        ...
        self.base_mut().emit_signal("inventories_initialized", &[]);
    }
}
...
impl GameSystem for InventoryManager {
    ...
    fn initialize() -> Gd<Self> {
        ...
        Engine::singleton().register_singleton(Self::NAME, &inventory_manager);
        inventory_manager
    }
