...
pub trait GoalBehaviour {
    fn validate_context(&self, goal: &GoalComponent, agent_world_context: &AgentGoalWorldContext) -> bool {
        let required_state_meet = goal.required_state.count_unsatisfied_world_state_props(agent_world_context.current_world_state) == 0;
        let desired_state_not_meet = goal.desired_state.count_unsatisfied_world_state_props(agent_world_context.current_world_state) != 0;
        required_state_meet && desired_state_not_meet
    }

    fn is_valid(&self, _goal: &GoalComponent, _agent_world_context: &AgentGoalWorldContext) -> bool {
        true
    }

    fn calculate_goal_relevance(&self, goal: &GoalComponent, _agent_world_context: &AgentGoalWorldContext) -> u32 {
        goal.priority
    }

    fn activate(&self, _goal: &GoalComponent, _agent_world_context: &mut AgentGoalWorldContext) -> bool { true }
