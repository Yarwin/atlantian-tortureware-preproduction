...
impl ItemSize {
    ...
    pub fn new_rectangular(width: usize, height: usize) -> Self {
        ...
        for row_i in 0..height {
            taken_space.push(RowSpace {
                col_offset: 0,
                row_offset: row_i,
                growth: width,
            });
        }
        ItemSize { space: taken_space }
    }
}
...
impl Grid {
    ...
    pub fn insert_item_at(&mut self, item_id: u32, at: usize, size: &ItemSize) -> InventoryResult {
    ...
    pub fn check_at(&self, at: usize, size: &ItemSize, item_id: Option<u32>) -> InventoryResult {
        let mut free = Vec::new();
        let mut taken = Vec::new();
        for row_space in size.space.iter() {
            match self.check_row_at(item_id, at, row_space) {
                InventoryResult::Free(f) => free.extend(f),
                InventoryResult::OutsideRange => return InventoryResult::OutsideRange,
...
mod test {
    ...
    fn test_creating_item_sizes() {
        ...
        for i in 0..excepted_item_sizes.len() {
            if excepted_item_sizes[i] != created_item_sizes[i] {
                panic!(
                    "failed creating rectangular item. Excepted: {:?}, given: {:?}",
                    excepted_item_sizes[i], created_item_sizes[i]
                )
            }
        }
    }
    ...
    fn test_inserting_space_free() {
        ...
        if !matches!(insert_result, InventoryResult::Inserted(_)) {
            panic!(
                "failed to insert an item. Excepted: InventoryResult::Inserted, actual: {:?}",
                insert_result
            );
        }
    ...
    fn test_inserting_first_free_space() {
        ...
        let InventoryResult::Free(first_free_space) = grid.get_first_free_space(&item_b) else {
        ...
        let InventoryResult::Free(first_free_space) = grid.get_first_free_space(&item_c) else {
            panic!("no free space found!")
        };
    ...
    fn test_inserting_space_taken() {
        ...
        if !matches!(insert_result, InventoryResult::Taken(_)) {
            panic!(
                "Wrong result! Excepted: InventoryResult::Taken, actual: {:?}",
                insert_result
            )
        }
    }
    ...
    fn test_inserting_outside_space() {
        ...
        if !matches!(insert_result, InventoryResult::OutsideRange) {
            panic!(
                "Wrong result! Excepted: InventoryResult::OutsideRange, actual: {:?}",
                insert_result
            )
        }
    }
    ...
    fn test_removing() {
        ...
        if !matches!(insert_result, InventoryResult::Inserted(_)) {
            panic!(
                "given space should be free! Excepted: InventoryResult::Inserted, actual: {:?}",
                insert_result
            )
        }
