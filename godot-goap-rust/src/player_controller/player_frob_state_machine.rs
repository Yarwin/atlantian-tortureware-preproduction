...
fn update_shapecast(player_frob_controller: &mut PlayerController, frob_data: &mut Option<FrobInfo>) {
    player_frob_controller.interface_shape_cast.force_shapecast_update();
...
fn disconnect_frobbed(player_frob_controller: &mut PlayerController, frobbed: Option<Gd<ActReactArea3D>>) {
    if let Some(mut frobbed) = frobbed {
        let on_shapecasted_freed = player_frob_controller.base().callable("on_shapecasted_freed");
        frobbed.disconnect("tree_exiting".into(), on_shapecasted_freed);
    }
}

fn stop_frobbing_and_disconnect(player_frob_controller: &mut PlayerController, frob_info: &mut Option<FrobInfo>) {
    if let Some(frob_info) = frob_info.as_mut() {
        frob_info.frobbed_since = None;
...
fn equip_item(player_frob_controller: &mut PlayerController, mut new_item: Gd<Item>, frob_info: Option<FrobInfo>) -> Box<dyn PlayerState> {
    {
        let mut item_bind = new_item.bind_mut();
        let Some(eq_component) = item_bind.equip.as_mut() else { panic!("no eq component for given item!"); };
        let (mut eq_component, ui) = eq_component.initialize_equipment_scene();
        drop(item_bind);
        eq_component.initialize(new_item.clone());
        player_frob_controller.eq_holder.add_child(&eq_component.base);
        GameSys::singleton().emit_signal("new_ui_item_equipped".into(), &[eq_component.clone().to_variant(), ui.to_variant()]);
...
fn handle_slot_change(player_frob_controller: &mut PlayerController, slot: u32, frob_info: Option<FrobInfo>) -> Result<Box<dyn PlayerState>, Option<FrobInfo>> {
    ...
    if let Some(new_item) = slot_item {
        if let Some(mut prev_item) = active_item {
            ...
            let Some(mut eq_component) = player_frob_controller.equipment_component.take() else {panic!("active item without eq component!")};
            eq_component.take_off();
            let on_item_taken_off = player_frob_controller.base().callable("on_old_item_taken_off");
...
pub trait PlayerState: Debug {
    fn handle_event(self: Box<Self>, player_frob_controller: &mut PlayerController, event: PlayerEvent) -> Box<dyn PlayerState>;
}
...
impl PlayerState for DefaultState {
    fn handle_event(mut self: Box<Self>, player_frob_controller: &mut PlayerController, event: PlayerEvent) -> Box<dyn PlayerState> {
        match event {
            ...
            PlayerEvent::FrobStopEvent => stop_frobbing(player_frob_controller, &mut self.frob_info),
...
impl GrabState {
    fn enter(mut item_grabbed: Gd<RigidBody3D>, player_controller: &mut PlayerController) -> Box<Self> {
        let on_grabbed_freed = player_controller.base().callable("on_grabbed_freed");
        item_grabbed.connect_ex("tree_exiting".into(), on_grabbed_freed).flags(CONNECT_ONE_SHOT).done();
        if let Some(equipment) = player_controller.equipment_component.as_mut() {
            equipment.point_down()
        }
        Box::new(GrabState {grabbed_ref: item_grabbed})
    }

    fn disconnect(&mut self, player_controller: &mut PlayerController) {
        let on_grabbed_freed = player_controller.base().callable("on_grabbed_freed");
        self.grabbed_ref.disconnect("tree_exiting".into(), on_grabbed_freed);
    }
    ...
    fn handle_event(mut self: Box<Self>, player_frob_controller: &mut PlayerController, event: PlayerEvent) -> Box<dyn PlayerState> {
        match event {
            PlayerEvent::ActivateEvent => {
                ...
                let Some(throw_effect) = player_frob_controller.throw_effect.as_mut() else {return self};
                let context = dict! {
                ...
                let effect = (command_init_fn)(throw_effect.clone().upcast::<Resource>(),
        ...
        match event {
            ...
            PlayerEvent::ActivateEvent => {
                if let Some(eq_component) = player_frob_controller.equipment_component.as_mut() {
                    ...
                    player_frob_controller.base().get_viewport().unwrap().set_input_as_handled();
                }
            }
            PlayerEvent::DeactivateEvent => {
                if let Some(eq_component) = player_frob_controller.equipment_component.as_mut() {
