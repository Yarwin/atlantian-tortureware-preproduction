...
impl GoalBehaviour for ChaseEnemyGoal {
    fn is_valid(&self, _goal: &GoalComponent, agent_world_context: &AgentGoalWorldContext) -> bool {
        let Some(AITarget::Character(..)) = agent_world_context.blackboard.target else {return false};
        true
    }

    fn activate(&self, _goal: &GoalComponent, agent_world_context: &mut AgentGoalWorldContext) -> bool {
        let Some(AITarget::Character(i, ..)) = agent_world_context.blackboard.target.as_ref() else {return false};
