...
impl GoalBehaviour for PatrolGoal {
    ...
    fn activate(
        ...
        if let Some(fact) = agent_world_context.working_memory.find_fact_mut(fact_query) {
            ...
            if let WMProperty::Node(Node::Patrol {
                ainode_id,
                position,
            }) = &mut fact.f_type
            {
                let ainodes_guard = agent_world_context
                    .ai_nodes
                    .as_mut()
                    .unwrap()
                    .read()
                    .expect("rwlock failed!");
                let ainode = ainodes_guard
                    .get(ainode_id)
                    .expect("no node with given id!");
                if ainode.is_locked() {
                    let fact_query =
                        FactQuery::with_check(FactQueryCheck::Node(WMNodeType::Patrol));
                    agent_world_context
                        .working_memory
                ...
                base.status
                    .store(*agent_world_context.id, Ordering::Release);
    ...
    fn deactivate(&self, _goal: &GoalComponent, agent_world_context: &mut AgentGoalWorldContext) {
        ...
        if let Some(ainode_id) = agent_world_context.blackboard.current_locked_node.take() {
            let Ok(ainodes_guard) = agent_world_context
                .ai_nodes
                .as_mut()
                .expect("no ainodes")
                .read()
